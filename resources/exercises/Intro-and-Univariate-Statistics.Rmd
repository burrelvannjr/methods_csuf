...
To merge these objects together, we use the <span style="color:blue">`cbind`</span> (or column bind) function -- which converts the list into a column (variable), and to convert this list of columns into a data set (AKA data frame), we use the <span style="color:blue">`as.data.frame`</span> function. 


```{r}
data <- as.data.frame(
  cbind(race,income,party)
  )
```
...
```{r}
data <- as.data.frame(cbind(race,income,party))
```
