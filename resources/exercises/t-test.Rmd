...
To run the t-test in R, we take the summary (output) of the analysis of variance <span style="color:blue">`aov`</span> function.


```{r}
summary(aov(data1$mpg ~ data1$am))
```
...
“Using an independent samples t-test, I reject/fail to reject the null hypothesis that there is no mean difference between women’s mean beers in the fridge and men’s mean beers in the fridge, in the population, $t(?) = ?, p ? .05$” 
