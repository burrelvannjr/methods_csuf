...
```{r}
histogram(~ mpg | am, data = data1, type = "count", main = "Histogram of 'Miles per Gallon' by 'Manual Transmission'")
```
...
```{r}
histogram(~ mpg | am, data = data1, main = "Histogram of 'Miles per Gallon' by 'Manual Transmission Type'", 
          type = "density", 
          panel=function(x, ...) {
            panel.histogram(x, ...)
...
```{r}
bwplot(~mpg | am, data = data1, pch="|", main = "Bar-and-Whisker Plot of 'mpg' by 'Manual Transmission Type'")
```
...
```{r}
xqqmath(~mpg | am, data=data1, fitline = TRUE, main = "Normal Q-Q Plot of 'Wr.Hnd' by 'Manual Transmission Type'", xlab = "Expected Normal")
```
...
To run the independent samples t-test in R, we take the summary (output) of the analysis of variance <span style="color:blue">`t.test`</span> function.
...
If you meet the assumptions of the t-test, you can **assume equal variances**, and therefore use the call <span style="color:blue">`var.equal=TRUE`</span> function. If you violate the assumptions, use the call <span style="color:blue">`var.equal=FALSE`</span> function.
