...
```{r}
xqqmath(~ mpg, data=data1, fitline = TRUE, main = "Normal Q-Q Plot of 'mpg'", xlab = "Expected Normal")
```

```{r}
xqqmath(~ wt, data=data1, fitline = TRUE, main = "Normal Q-Q Plot of 'wt'", xlab = "Expected Normal")
```
...
* Not having groups that have extremely different values (e.g. for which a t-test/ANOVA might appropriately identify)
  + Examine means for various groups in the sample
...
For Chi Square, within the <span style="color:blue">`cor.test`</span> function, the independent variable is listed first and the dependent variable is listed second. 
